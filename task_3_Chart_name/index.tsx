...
const SliceInfo = ({sliceId}) => {
	...
	// SOLUTION
	const layout = useSelector((state: RootState) => state.dashboardLayout.present);
	
	const layoutItem = Object.values(layout).find(item => item.meta.chartId === sliceId);
	
	const customChartName = layoutItem?.meta.sliceNameOverride || layoutItem?.meta.sliceName || 'Имя графика не найдено';
